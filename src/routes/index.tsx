...
function Predictrisk(form: PatientForm): Promise<Prediction> {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const chol = Number(form.cholesterol);
      ...
      if (form.activity === "inactive") score += 1;
      if (form.history === "yes") score += 2;

      const level: RiskLevel = score >= 9 ? "HIGH" : score >= 4 ? "MEDIUM" : "LOW";

      // Distribute mock probabilities around the predicted level.
      const jitter = (n: number) => Math.max(0, Math.min(100, n));
      let probs: Prediction["probabilities"];
      if (level === "HIGH") {
        const high = jitter(88 + ((score * 7) % 10));
        const medium = jitter(((100 - high) * 3) / 4);
        probs = { high, medium, low: 100 - high - medium };
      } else if (level === "MEDIUM") {
        const medium = jitter(70 + ((score * 11) % 15));
        const high = jitter(((100 - medium) * 2) / 3);
        probs = { high, medium, low: 100 - medium - high };
      } else {
        const low = jitter(90 + ((score * 5) % 8));
        const medium = jitter(((100 - low) * 2) / 3);
        probs = { high: 100 - low - medium, medium, low };
      }
      resolve({ level, probabilities: probs });
    }, 1400);
  });
}
